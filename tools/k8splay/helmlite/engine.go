...
// Adapted from Helm (https://github.com/helm/helm, Apache License 2.0) for the
// Kubernetes playground: the cluster-facing parts (lookup via client-go,
// schema validation) are removed so the engine compiles small to WebAssembly.
...
func (e Engine) Render(chrt *chart.Chart, values Values) (map[string]string, error) {
...
func Render(chrt *chart.Chart, values Values) (map[string]string, error) {
...
func (e Engine) render(tpls map[string]renderable) (rendered map[string]string, err error) {
	...
	for _, filename := range keys {
		...
		vals["Template"] = Values{"Name": filename, "BasePath": tpls[filename].basePath}
		var buf strings.Builder
...
func allTemplates(c *chart.Chart, vals Values) map[string]renderable {
...
// scope.
func recAllTpls(c *chart.Chart, templates map[string]renderable, vals Values) map[string]interface{} {
