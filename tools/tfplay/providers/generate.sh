...
#
#   AWS_VERSION=6.66.0 RANDOM_VERSION=3.9.1 NULL_VERSION=3.3.2 LOCAL_VERSION=2.9.1 GOOGLE_VERSION=8.4.0 ./generate.sh
set -euo pipefail
...
GOOGLE_VERSION="${GOOGLE_VERSION:-8.4.0}"
...
for p in "aws:$AWS_VERSION" "random:$RANDOM_VERSION" "null:$NULL_VERSION" "local:$LOCAL_VERSION" "google:$GOOGLE_VERSION"; do
  name="${p%%:*}"; ver="${p##*:}"
...
src_dir="$(# Same for terraform-provider-google (module path lacks the /vN suffix too).
gsrc_dir="$(cd "$here/.." && go mod download -json github.com/hashicorp/terraform-provider-google@main | sed -n 's/.*"Dir": "\(.*\)".*/\1/p')"

cd "$here/.." && go mod download -json "github.com/hashicorp/terraform-provider-aws@v${AWS_VERSION}" 2>/dev/null | sed -n 's/.*"Dir": "\(.*\)".*/\1/p' || true)"
if [ -z "$src_dir" ]; then
...
go run ./cmd/schemagen -schema "$work/schema.json" -provider registry.terraform.io/hashicorp/google \
  -name google -version "$GOOGLE_VERSION" -src "$gsrc_dir" -google-iam -meta "$here/meta/google.json" -out "$out/google.json.gz"
