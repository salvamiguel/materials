...
:::tip Pruébalo sin clúster
El [playground de Kubernetes](/k8s-playground) simula un ArgoCD ligero: los mismos comandos de esta sección, la UI web (pestaña «Navegador») y el CLI `argocd`, contra un repositorio Git simulado en el que haces `git commit` y `git push`. Sus pods son pods de sistema y la reconciliación ocurre dentro de `argocd-application-controller-0`.

<K8sPlaygroundLink example="argocd">Abre el repo de clase gitops-status-demo-config en el playground</K8sPlaygroundLink>
:::
...
<LabActions title="Repositorio de la configuración (kustomize & argocd)" repo="https://github.com/salvamiguel/gitops-status-demo-config" codespace={false} fork={false} k8s={true} />
