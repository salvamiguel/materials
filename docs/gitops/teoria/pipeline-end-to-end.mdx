...
:::tip Repositorios reales
Este pipeline esta implementado en repositorios reales que puedes clonar y ejecutar:
- **App repo**: [gitops-status-demo-app](https://github.com/salvamiguel/gitops-status-demo-app) — contiene el codigo Go y el workflow de GitHub Actions
- **Config repo**: [gitops-status-demo-config](https://github.com/salvamiguel/gitops-status-demo-config) — contiene los manifiestos Kustomize y las Applications de ArgoCD

Sigue las instrucciones del README del config repo para montar el entorno local con `kind`.
:::
...
jobs:
  ...
  update-manifests:
    ...
    steps:
      - uses: actions/checkout@v5
        with:
          repository: salvamiguel/gitops-status-demo-config    # Config repo separado
...
```mermaid
graph LR
  subgraph "App Repo (salvamiguel/gitops-status-demo-app)"
    src["src/<br/>main.go, ..."]
    Dockerfile
  ...
  subgraph "Config Repo (salvamiguel/gitops-status-demo-config)"
...
kubectl get events -n status-prod --watch

# Ver eventos ordenados por tiempo
kubectl get events -n status-prod --sort-by='.lastTimestamp'

# Ver logs del pod que está fallando
kubectl logs -n status-prod -l app=gitops-status-demo --tail=100 -f

# Ver el estado detallado del deployment
kubectl describe deployment gitops-status-demo -n status-prod
```
...
- name: Open PR to promote to staging
  ...
  with:
    ...
    commit-message: "chore: update gitops-status-demo to ${{ github.sha }}"
    title: "Promote gitops-status-demo ${{ github.sha }} to staging"
    body: |
      ## Promotion Request
      ...
      **Source**: ${{ github.repository }}@${{ github.sha }}
      ...
      - [ ] Revisión de cambios
      - [ ] Validado en dev
    branch: promote/gitops-status-demo-${{ github.sha }}
    base: main
```
...
metadata:
  ...
  annotations:
    ...
    argocd-image-updater.argoproj.io/image-list: gitops-status-demo=ghcr.io/salvamiguel/gitops-status-demo-app
    argocd-image-updater.argoproj.io/gitops-status-demo.update-strategy: semver
    argocd-image-updater.argoproj.io/gitops-status-demo.allow-tags: regexp:^v[0-9]+\.[0-9]+\.[0-9]+$
